...
ab_match = ab_regex.search('Recognize some letter: a is my string') # search() looks for a match in the whole string 
print(f'{ab_match = }')
if ab_match:
...
email_regex = re.compile(r'^[a-z]+@[a-z]+\.[a-z]+$') # $ - end of the string, ^ - start of string
# r - raw string (i.e. dont process any backslashed characters)
...
print('All prices: ', price_re.findall('The sale price of $199 is worth it, but i got it for $49.99 which was a great deal.'))

test_re = re.compile('(0|1)(1+)?(0)(0)?(1)')

test_match = test_re.search("001")
if test_match:
    print(f'{test_match.group() = }')
else:
    print('No match found')

test_match = test_re.search("000")
if test_match:
    print(f'{test_match.group() = }')
else:
    print('No match found')

test_match = test_re.search("1111000")
if test_match:
    print(f'{test_match.group() = }')
else:
    print('No match found')
