...
void activateSpace(int mouse_number, int &mouse_position, int &mouse_cheese){
    //TODO: COMPLETE THE MISSING PARTS OF THIS FUNCTION
    
    cout << "Landed on: ";
    switch(determineSpaceType(mouse_position)){
        case 0: {
            //TODO: Build 2 or Build 3 or Build 4
            //Steps:
            //Print out which Build Number it is
            //If the number of players is equal or less than the build number,
            //then add a component to the trap
            
            cout << "Build " << determineX(mouse_position) << ".\n";
            if (player_count <= determineX(mouse_position)){
                //If a component was added to the trap, then take a cheese from the supply
                if (assembleTrap() == true) {
            ...
            // Landed on: Build 2.
            // Adding component #1: Base A inserted.
            // Took 1 cheese pieces from the supply.
        }
        case 1: {
            ...
            // Landed on: Move Forward 4.
            // Moving 4 steps forward...
        }
        case 2: {
        ...
        case 3: {
            ...
            // Landed on: Take 2 Cheese.
            // Took 2 cheese pieces from the supply.
        }
        case 4: {
            //TODO: Lose Cheese
            //Steps: 
            //Print how much cheese is lost.
            //Deduct that many cheese from the current mouse and return it to the supply
            //If the mouse doesn't have that much cheese, take all of it instead.
            cout << "Lose " << determineX(mouse_position) << " Cheese.\n";
            mouse_cheese -= determineX(mouse_position);
            if (mouse_cheese < 0){
                cheese_supply += determineX(mouse_position) + mouse_cheese;
                mouse_cheese += (mouse_cheese * -1);
            }
            else{
                cheese_supply += determineX(mouse_position);
            }            
            break;
            
            // Landed on: Lose 2 Cheese.
        }
        case 5: {
            cout << "Dog bone! Nothing happens." << endl;
            break;
            
            // Landed on: Dog bone! Nothing happens.
        }
        case 6: {
            cout << "Take 2 Cheese.\n";
        ...
        case 7: {
            //TODO: Double Build 4
            //Build 4 twice, following the steps
            //illustrated in Case 0
            cout << "Double Build 4.\n";
            if (player_count <= 4){
                if (assembleTrap() == true) {
                    takeCheese(1,mouse_number);
            ...
            // Landed on: Double Build 4.
            // Adding component #1: Base A inserted.
            // Took 1 cheese pieces from the supply.
            // Adding component #2: Gear Support attached to Base A.
            // Took 1 cheese pieces from the supply.
        }
        case 8: {
            cout << "Safe!" << endl;
        ...
        case 9: {
            //TODO: Complete Turn Crank (Don't do this until you've finished the other cases!)
            ...
            if (trap_components_remaining > 0){
                ...
                while (more_rolls && mouse_cheese > 0){
                    ...
                    //If they say Y then ask them which other mouse to move
                    //If the mouse they want to move is still in the game 
                    //and isn't on the safe spot on position 44,
                    //Then return a cheese to the supply, move that mouse, and redraw the board.
                    //Otherwise they are done.
                    cin >> answer1;
                    if (toupper(answer1) == 'Y'){
                        cout << "Which mouse will you move?" << endl;
                        cin >> answer2;
                        if ((mouse_in_game[answer2]==true) && (mouse_positions[answer2] != 44)){
                            cheese_supply ++;
                            mouse_cheese --;
                            moveMouse(answer2);
                            drawBoard();
                ...
                //TODO: Attempt to activate the trap
                //If it is successfully activated, 
                //Then for each mouse on the Cheese Wheel,
                //Print that the mouse was captured and remove it from the game,
                //and give that mouse's cheese to the current player.
                if (activateTrap() == true) {
                    for (int i = 0; i < 4; i++){
                        if (mouse_positions[i]== 49){
                            cout << "mouse " << i+1 << " has been trapped.\n";
            ...
            // Landed on: Turn Crank.
            // Cannot turn crank; Trap is incomplete.
            // Moving Mouse 0!
            // Rolled: 4
            // Turning the crank...
            // Gears are rotating...
            // The lever pushed the stop sign!
            // The stop sign pushed the boot!
            // The boot tipped over the bucket!
            // The marble has been released!
            // The marble is descending the stairs...
            // The marble is rolling down the rain gutter...
            // The marble struck the rod with the helping hands!
            // The second marble is released!
            // The marble has fallen into the bathtub!
            // The marble has hit the diving board!
            // The diver has been launched into the air!
            // The diver landed in the washtub!
            // The cage is falling!
            // The cage got stuck! The trap has failed.
        }
        default:
...
void playGame(){
    ...
    cout << "How many pleyers?\n";
    cin >> player_count;
    for (int i = 0; i<player_count; i++){
        mouse_in_game[i] = true;
    }
    while(player_count > 1){
        for (int i = 0; i<player_count; i++){
            cout << "mouse " << i << "'s turn\n";
            cout << "There are " << cheese_supply << " cheese in the cheese supply\n";
            cout << "You have " << mouse_cheeses[i] << " cheese\n";
            moveMouse(i);
...
int main(){
    srand(time(NULL));
    // player_count = 2;
    // mouse_in_game[0] = true;
    // mouse_in_game[1] = true;
    // drawBoard();
    // mouse_positions[0] = 46;
    // mouse_cheeses[0] = 10;
    // trap_components_remaining = 0;
    // activateSpace(0, mouse_positions[0], mouse_cheeses[0]);
    // cout << "cheese: "<< mouse_cheeses[0] << endl;
    playGame();
    return 0;
}
